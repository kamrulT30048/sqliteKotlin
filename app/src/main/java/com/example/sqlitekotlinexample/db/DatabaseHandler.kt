...
import com.example.sqlitekotlinexample.Common
...
class DatabaseHandler(context: Context)
    ...
    DatabaseHandler.DB_NAME, null,
    DatabaseHandler.DB_VERSION) {

    override fun onCreate(db: SQLiteDatabase) {
        val CREATE_TABLE =
            "CREATE TABLE $TABLE_NAME ($ID INTEGER PRIMARY KEY, $NAME TEXT, $DESC TEXT, $COMPLETED TEXT, $END_DATE LONG);"
    ...
    fun addTask(tasks: Tasks): Boolean {
        ...
        values.put(END_DATE, tasks.end_date)
...
//        taskObj.start_date = cursor.getString(cursor.getColumnIndex(START_DATE))
        taskObj.end_date = cursor.getString(cursor.getColumnIndex(END_DATE)).toLong()
        cursor.close()
        db.close()
        return taskObj
    }
    ...
    fun getAllTasks(): List<Tasks> {
        ...
        val db = writableDatabase
        val selectQuery = "SELECT  * FROM $TABLE_NAME ORDER BY $END_DATE ASC"
        val cursor = db.rawQuery(selectQuery, null)
        if (cursor != null) {
...
//                    tasks.start_date = cursor.getString(cursor.getColumnIndex(START_DATE))
                    tasks.end_date = cursor.getString(cursor.getColumnIndex(END_DATE)).toLong()
                    taskList.add(tasks)
    ...
    fun updateTask(tasks: Tasks): Boolean {
        ...
        values.put(DESC, tasks.desc)
        values.put(COMPLETED, tasks.completed)
//        values.put(START_DATE, tasks.start_date)
        values.put(END_DATE, tasks.end_date)
    ...
    companion object {
        ...
        private val END_DATE = "DueDateTime"
    }
}